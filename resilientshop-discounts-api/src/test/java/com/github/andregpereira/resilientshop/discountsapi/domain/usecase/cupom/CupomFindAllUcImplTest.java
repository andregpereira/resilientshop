...
class CupomFindAllUcImplTest {

    @InjectMocks
    private CupomFindAllUcImpl findAllUc;

    @Mock
    private CupomGateway gateway;

    @Test
    void findAll_ReturnsCupomPage() {
        given(gateway.findAll(any(Pageable.class))).willReturn(PAGE_CUPOM);
        Page<Cupom> sut = findAllUc.findAll(PAGEABLE_ID);
        assertThat(sut).isNotEmpty().hasSize(1).isEqualTo(PAGE_CUPOM);
    }

    @Test
    void findAll_ReturnsEmptyPage() {
        given(gateway.findAll(any(Pageable.class))).willReturn(Page.empty());
        Page<Cupom> sut = findAllUc.findAll(PAGEABLE_ID);
